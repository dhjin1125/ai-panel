...
- `claude`, `gemini`, `codex` CLI가 설치되어 있고 로그인되어 있어야 합니다.
- v1은 텍스트 논제만 다룹니다.
...
```bash
cd /Users/jdh/Desktop/project/ai-panel-cli
PYTHONPATH=src python3 -m ai_panel --help
PYTHONPATH=src python3 -m ai_panel ask examples/topic.md
PYTHONPATH=src python3 -m ai_panel debate examples/topic.md
./ai-panel serve
```
...
화면에서는 `논제`를 입력하고 `비교` 또는 `토론`을 선택한 뒤 `실행`을 누르면 됩니다. 기본 결과 화면은 Claude/Gemini/Codex 답변을 3컬럼으로 보여주고, 개별 md/json 탭을 누르면 원본 파일만 따로 볼 수 있습니다.

특정 CLI가 실패하면 결과 화면에 실패한 모델과 직접 확인할 명령이 표시됩니다. 터미널에서 해당 CLI 로그인/세션을 확인한 뒤 `같은 토픽 다시 실행`을 누르면 됩니다.

왼쪽 `CLI 연동` 영역의 `연동` 버튼을 누르면 해당 CLI의 로그인/세션 확인 터미널이 열립니다. Claude는 `claude auth`, Gemini는 `gemini`, Codex는 `codex login`을 실행합니다.

각 CLI 연동 영역에서 사용할 모델도 선택할 수 있습니다. `CLI default`는 해당 CLI가 현재 계정/설정 기준으로 자동 선택하는 모델이라는 뜻입니다. 다른 항목을 고르면 실행 명령에 `--model <모델>` 옵션을 붙입니다.
...
```bash
PYTHONPATH=src python3 -m ai_panel ask topic.md
PYTHONPATH=src python3 -m ai_panel debate topic.md
PYTHONPATH=src python3 -m ai_panel show <run-id>
PYTHONPATH=src python3 -m ai_panel doctor
PYTHONPATH=src python3 -m ai_panel serve
```
